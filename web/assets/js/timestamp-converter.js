...
function convertTimestampsToLocalTimezone() {
    document.querySelectorAll('.server-timestamp').forEach(function (element) {
        ...
        const targetTimezone = element.getAttribute('data-timezone') || Intl.DateTimeFormat().resolvedOptions().timeZone;
        const showMilliseconds = element.hasAttribute('data-precision') && element.getAttribute('data-precision') === 'ms';
        ...
        if (serverTimestamp) {
            try {
                ...
                const goFormatMatch = serverTimestamp.match(/^(\d{4}-\d{2}-\d{2})\s(\d{2}:\d{2}:\d{2})(?:\.(\d+))?\s([+-]\d{4})\s\w+/);
                if (goFormatMatch) {
